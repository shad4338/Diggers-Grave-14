...
public sealed partial class GunSystem : SharedGunSystem
{
        ...
        EntityCoordinates fromCoordinates, EntityCoordinates toCoordinates, out bool userImpulse, EntityUid? user = null, bool throwItems = false)
    {
        ...
        // DG Start
        // Add movement-based spread
        var extraSpread = Angle.Zero;
        if (user != null && TryComp<PhysicsComponent>(user.Value, out var physics) && physics.LinearVelocity.Length() > 0.1f)
        {
            extraSpread = Angle.FromDegrees(gun.Ergonomics);
        }

        var angle = GetRecoilAngle(Timing.CurTime, gun, mapDirection.ToAngle(), user, extraSpread);
        // DG End
    ...
    private Angle GetRecoilAngle(TimeSpan curTime, GunComponent component, Angle direction, EntityUid? user, Angle extraSpread = default)
    {
        ...
        var angle = new Angle(direction.Theta + component.CurrentAngle.Theta * random + extraSpread.Theta);
